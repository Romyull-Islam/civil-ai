...
export default function Page() {
  const [site, setSite] = useState<{ refundPolicy: string; appName: string; companyName: string; companyAddress: string; supportEmail: string } | null>(null);
  ...
  return (
    <div className="h-full overflow-y-auto"><div className="max-w-3xl mx-auto p-6 grid gap-4">
      ...
      {site && <div className="card p-5"><Markdown text={site.refundPolicy} />{(site.companyName || site.companyAddress || site.supportEmail) && <p className="text-xs text-muted mt-4">{site.companyName}{site.companyAddress ? ` · ${site.companyAddress}` : ""}{site.supportEmail ? ` · ${site.supportEmail}` : ""}</p>}</div>}
