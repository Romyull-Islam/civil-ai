...
export async function POST(req: Request) {
  ...
  const b = (await req.json()) as { email: string; plan: string; txnId: string; amount?: number; currency?: string; method?: string; days?: number; seats?: number };
  ...
  const r = await activatePlan(user.id, b.plan, b.days, b.seats);
  if (!r) return Response.json({ error: "Unknown plan" }, { status: 400 });
  const id = newId();
  await db.createPayment({ id, userId: user.id, email: user.email, plan: b.plan, method: b.method ?? "webhook", amount: Number(b.amount ?? 0), currency: b.currency ?? "BDT", txnId: b.txnId, sender: "", status: "approved", note: "auto (webhook)", createdAt: Date.now(), reviewedAt: Date.now(), seats: b.seats ?? 1 });
