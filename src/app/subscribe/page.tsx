...
interface SiteInfo {
  ...
  plans: { id: string; name: string; priceMonthly: number; currency: string; periodDays: number; features: string[]; dailyRequests: number; perSeat?: boolean; minSeats?: number }[];
}
          ...
          <aside className="card p-5 grid gap-3 text-sm lg:sticky lg:top-4">
            ...
            {plan ? (
              <>
                ...
                <div className="flex justify-between text-muted text-xs"><span>AI requests</span><span>{plan.dailyRequests} / day{plan.perSeat ? " per user" : ""}</span></div>
