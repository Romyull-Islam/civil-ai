  ...
  return (
    <div className="h-full overflow-y-auto"><div className="max-w-2xl mx-auto p-6 grid gap-4">
      ...
      <div className="card p-4 grid gap-2 text-sm">
        ...
        <div><span className="label">AI credits today</span><div>{s.usage?.used}{s.usage?.limit != null ? ` / ${s.usage.limit}` : " (unlimited)"}</div></div>
        <div><span className="label">AI credits this period</span><div>{s.usage?.periodUsed ?? 0}{s.usage?.periodLimit != null ? ` / ${s.usage.periodLimit}` : " (unlimited)"}{s.usage?.periodStart ? <span className="text-xs text-muted"> since {s.usage.periodStart}</span> : null}</div></div>
