...
import { MessageSquare, Calculator, PencilRuler, BookOpen, Settings, HardHat, Menu, X, Shield, User, CreditCard, LifeBuoy, Users, Cloud, ChevronsLeft, ChevronsRight } from "lucide-react";
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-full">
      <aside className={`${open ? "flex" : "hidden"} md:flex ${collapsed ? "md:w-14" : "w-60"} shrink-0 flex-col border-r border-border bg-elev fixed md:static inset-y-0 left-0 z-40 transition-[width]`}>
        <div className={`flex items-center gap-2 ${collapsed ? "px-2 justify-center" : "px-4"} py-4 border-b border-border`}>
        ...
        <button className="hidden md:flex items-center justify-center gap-2 mx-2 my-1 rounded-lg py-1.5 text-xs text-muted hover:bg-elev2 hover:text-fg" onClick={toggleCollapsed} title={collapsed ? "Expand menu" : "Collapse menu"}>{collapsed ? <ChevronsRight size={16} /> : <><ChevronsLeft size={16} /> Hide menu</>}</button>
        {saas && session?.user && !collapsed && <div className="px-3 py-2 text-xs text-muted border-t border-border truncate flex items-center gap-2">{session.avatar && <Avatar src={session.avatar} />}<span className="truncate">{session.user.email} · <span className="text-fg">{session.plan?.name}</span>{session.usage?.limit != null ? ` · ${session.usage.remaining} left today` : ""}</span></div>}
        {!collapsed && <div className="mt-auto p-3 text-[11px] text-muted leading-snug border-t border-border">
