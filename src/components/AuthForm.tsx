...
export function AuthForm({ kind }: { kind: "login" | "signup" }) {
  ...
  const company = session?.mode === "company";
  ...
  return (
    <div className="min-h-full flex items-center justify-center p-6">
      <form onSubmit={submit} className="card p-6 w-full max-w-sm grid gap-3">
        ...
        <h1 className="font-medium">{kind === "login" ? "Sign in" : company ? "Create the administrator account" : "Create your account"}</h1>
        {company && kind === "login" && session?.setupNeeded && <div className="text-sm rounded-lg border border-accent/40 bg-accent/10 px-3 py-2">First start: <Link className="text-accent2 underline" href="/signup">create the administrator account</Link>. After that, the administrator adds everyone else.</div>}
        ...
        {company ? <div className="text-xs text-muted text-center">{kind === "login" ? <>No account? Ask your administrator. · <Link className="text-accent2" href="/forgot">Forgot password?</Link></> : <Link className="text-accent2" href="/login">Sign in</Link>}</div> : <div className="text-xs text-muted text-center">{kind === "login" ? <>No account? <Link className="text-accent2" href="/signup">Sign up</Link> · <Link className="text-accent2" href="/forgot">Forgot password?</Link></> : <>Have an account? <Link className="text-accent2" href="/login">Sign in</Link></>} · <Link className="text-accent2" href="/pricing">Plans</Link></div>}
