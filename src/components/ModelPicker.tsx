...
function PlanModelChooser() {
  ...
  return (
    <div className="relative flex items-center gap-2 text-[12px] text-muted" ref={ref}>
      ...
      {session?.usage?.remaining != null && <Link href="/usage" className="hidden sm:inline hover:text-fg" title="Session, weekly and monthly usage">{session.usage.remaining} credits left</Link>}
      {open && (
        <div className="absolute bottom-full left-0 mb-2 w-80 max-h-[60vh] overflow-y-auto card p-1 shadow-xl z-50 text-sm">
          ...
          {session?.usage && session.usage.remaining !== null && <div className="border-t border-border mt-1 px-3 py-2"><UsageBars usage={session.usage} compact now={now} /></div>}
          {locked.length > 0 && (
            <>
