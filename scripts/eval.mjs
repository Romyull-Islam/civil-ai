...
const TASKS = [
  { id: "beam-udl", prompt: "Simply supported beam, 6 m span, 25 kN/m factored UDL. What are the maximum bending moment and shear?", tools: ["analyze_beam"], check: (o) => near(o.analyze_beam?.result?.maxMomentPositive?.value, 112.5) && near(Math.abs(o.analyze_beam?.result?.maxShear?.value), 75) },
  ...
  { id: "concrete-qty", prompt: "How many bags of cement, and how much sand and aggregate, for 12 m³ of M20 concrete?", tools: ["concrete_materials"], check: (o) => { const b = o.concrete_materials?.result?.cement?.bags; return b >= 96 && b <= 110; } },
